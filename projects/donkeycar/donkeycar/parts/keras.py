...
class Keras3D_CNN(KerasPilot):
    def __init__(self, image_w =160, image_h=120, image_d=3, seq_length=3, num_outputs=2, roi_crop=(0, 0), *args, **kwargs):
...
#时间序列模型类
class ConvLSTM2DPilot(KerasPilot):
    def __init__(self, model=None, image_w=160, image_h=120,   
image_d=3, seq_length=3, num_outputs=2, *args, **kwargs):
        ...
        if model:
            ...
            # 使用LSTM 2D的机器学习模型
            self.model = self.default_lstm()
        # 是否输出模型结构
        #print(self.model.summary())

    def run(self, img_array):

        # 通过get_current_img_arrays()获得输入图像的序列进行预测
        self.img_seq = get_current_img_arrays(self.seq_length, img_array, self.img_seq)

        # 数据创建
        img_arr = np.reshape(self.img_seq, (1, self.seq_length) + self.image_shape)
        # 预测执行
        outputs = self.model.predict(img_arr)
        steering = outputs[0]
        throttle = outputs[1]
...
def get_current_img_arrays(seq_length, img_array, prev_img_arrays):
    '''
     作用：转换为 时序模型 的图像序列，就是把原来一帧一帧的图像捆绑成长度为seq_length的序列输入到到神经网络中。
     参数：
         seq_length：序列长度，本例中序列长度为3，即神经网络按3帧图像处理。
         img_array：最新的图像数组
         prev_img_arrays：过去序列数的图像数组列表
    返回值：
         最新图像阵列列表
    '''
